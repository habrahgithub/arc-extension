...
The first-run/bootstrap flow (U01–U06) now guides new operators through:

1. **Governed Root selection** — workspace root / active file root / nested project root
2. **Safe config bootstrap** — explicit `.arc/router.json` + `.arc/workspace-map.json` creation with fail-closed defaults
3. **Blueprint bootstrap** — template-driven, generalized naming, non-authorizing
4. **Task Board rebinding** — Task Board rebinds to the selected governed root

The canonical operator SOP remains:
